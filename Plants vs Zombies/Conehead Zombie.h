...
public:
	sf::Texture zombie_texture[63];
	sf::Clock textureClock;
public:

	Conehead_Zombie(patch grid[][9],
		string path = "Assets/Images/Zombies/conehead zombie/conehead ("
	) {

		//srand(time(0));
		number_of_textures = 63;
	...
	void move_zombie() {
		...
		if (textureClock.getElapsedTime().asSeconds() >= 0.05f) {
			// Change texture
	...
	Conehead_Zombie& operator=(const Conehead_Zombie& other) {
